...
**`npm test` is expected to be fully green (376 passed).** No `BUG:`-tagged known-failing tests are outstanding — the CSV fee round-trip defect that used to hold the count at 1 failure is fixed. If a future defect lands a new `BUG:` test, [TESTING.md](TESTING.md) and [KNOWN_ISSUES.md](KNOWN_ISSUES.md) carry the expected count; a `BUG:` test must never be made green by rewriting its expectation. Any failure today is a real regression.
...
- `src/App.test.jsx` — component smoke tests for the trade form's validation gate, the trades table, the journal panel, the strategy playbook, the cashflow tab, the journal-timezone picker, the login gate and the render-level `ErrorBoundary` (jsdom via per-file pragma; the lib suite stays DOM-free). `TradeForm`, `TradesTable`, `JournalPanel`, `PlaybookPanel`, `CashflowPanel`, `TimezonePicker`, `HelpPanel`, `AuthGate`, `SettingsPanel` and `ErrorBoundary` are exported from App.tsx for these tests only. This test file itself stays `.jsx` — it drives already-typed exports rather than adding types of its own.
...
- `src/lib/format.ts` — pure display formatting shared by both bundles (`src/lib/format.test.js`).
- `src/lib/reportchart.ts` — chart primitives for the **exported** reports, as plain SVG/HTML strings: `svgLineChart`, `svgBarChart`, `svgDonut`, `chartLegend`, and `htmlBarRows` (coloured table cells — Word drops inline SVG silently, so the Word path never emits any). No React, no DOM, no measurement. Colours are literal, not tokens: a report is read outside the app. Covered by `src/lib/reportchart.test.js`.
- `src/lib/report.ts` — the exported report *documents*: `reportChartData` (the four series every report charts), `reportChartsHtml` (that section in whichever rendering the target engine can draw) and `interactiveReportHtml` (the whole self-contained interactive `.html`, script included). App.tsx owns *saving* a report; this owns what goes in it. Covered by `src/lib/report.test.js` (markup, escaping) and `src/lib/report.dom.test.js` (the shipped script's own search/filter/sort behaviour, run in jsdom).
- `src/lib/storage.ts` — the storage backend switch, typed `Storage` interface (`get/set/delete/list`).
- `src/App.tsx` (~5.5k lines) — the React shell: design tokens, CSS, every panel and modal, and the root `App` component. Sections are marked with `/* ==== NAME ==== */` banners; grep those to navigate. List-scale code paths (e.g. the trades table sort) derive per-row keys once and keep comparators free of per-comparison parsing — hold that line when touching them.
...
**Import direction is one-way: `App.tsx` → `lib/*`.** `Charts.tsx` and every `lib/*` module (`format.ts`, `trade.ts`, `report.ts`, `reportchart.ts`, …) must never import `App.tsx` — that circular import drags the whole app into the lazy chart chunk and defeats the code split. Enforced by a `no-restricted-imports` rule in `eslint.config.js`, which applies to `lib/**/*.ts` the same as `.js`/`.jsx`.

New pure logic goes in `lib/trade.ts` with a test, not in App.tsx. Anything touching `window`, `document` or `storage` stays in App.tsx. A body of pure logic large enough to bury `trade.ts` gets its own `lib/` module instead (that is what `report.ts`/`reportchart.ts` are) — the rule is "pure, in lib, with a test", not "everything in one file".
...
- **Journal timezone** — `settings.timezone` (IANA id, `""` = follow the machine = pre-v3.2 behaviour). Moves **only "now"** (today keys, presets, calendar highlight, clock, picker prefill) via `zonedNow()`; stored trade times are naive wall-clock strings and are never shifted. Reaches leaf components via `TimezoneContext` in App.tsx, whose `""` default keeps provider-less component tests on legacy behaviour. `tzOffsetLabel()` renders the live GMT offset (DST-aware) on the topbar clock. The Settings picker is `TimezonePicker` — a searchable combobox, not a `<select>`: ~400 zones ordered **ascending by offset** under one sticky `+HH:MM GMT` heading per offset (offset first, so the column lines up on the sign) (`offsetLabelPadded`, so the list reads as an even :00/:30 ladder plus the real :45 zones), searchable by city, region or offset in any of `gmt+5:30` / `+5:30` / `5:30` / `05:30`. The shaping is pure — `timezoneOptions` / `filterTimezoneOptions` / `groupTimezoneOptions` in `lib/trade.ts` — and the component only does the widget. See [ARCHITECTURE.md](ARCHITECTURE.md#the-journal-timezone).
...
- **Strategy playbook** — `settings.strategyNotes`, name → free text, normalized by `normalizeStrategyNotes` (plain string map, blanks dropped, 5000-char cap). Lives on its **own Playbook tab** (moved out of Settings in 3.3). A note deliberately survives its strategy being renamed or removed — the name coming back finds it intact. Each note sits under that strategy's actual record (`strategyPerformance` in `lib/trade.ts`: trades, open count, win rate, net, avg RR, profit factor, last close, plus a win/loss bar), with search over names *and* note text and sorts by trades / net / win rate. An untraded strategy still gets a row — the note is the point — and sorts last under the result-based sorts rather than ranking as a zero.
...
- **Login gate** — opt-in, soft: `AUTH_KEY` (`brij-tj-auth-v1`) holds `{users:[{id,username,salt,hash,displayName?,avatar?,lastLoginAt?}], allowSignup}`, kept **out of the meta blob** so password hashes never ride along in a journal backup — and so restoring someone else's backup can't switch self-signup on. `lib/auth.ts` hashes with PBKDF2-SHA-256; no plaintext is stored. Zero users = no gate (backward compatible). Creating the first account (Settings > Security) turns the gate on and signs the creator in; removing the last user turns it off. `signedIn` is derived (not stored), so removing the session's user re-shows the gate with no effect. Session lives in memory only — closing the app signs out. **Multi-user and web-future ready:** the store is a list, and swapping `AuthGate`'s local `verifyPassword` for a network call leaves the shell unchanged (see [ARCHITECTURE.md](ARCHITECTURE.md#authentication)). It gates the running app, **not** the files on disk — not disk encryption.
- **Sign-up & Profile** — `AuthGate` has Sign in / Create account tabs; the create path is gated on `allowSignup`, **off by default** (a gate anyone can register past is not a gate) except when there are zero accounts. The **Profile tab** shows only while signed in, is appended to `NAV` so `Ctrl/Cmd+1–9` stays put, and owns display name / avatar / password change / sign out; Settings > Security keeps *administration* (who may sign in, and the signup toggle). `displayName`, `avatar` and `lastLoginAt` are additive with fallbacks — `displayNameOf()` falls back to the username, and the **username itself is never editable** (every record is keyed to it). `changePassword()` verifies the current password first and draws a fresh salt.
...
Several inputs set `outline: none` and signal focus with a 1px border tint, which a keyboard user cannot reliably see. One rule at the top of `APP_CSS` puts an accent ring on `:focus-visible` for everything (and on `:focus-within` for the wrappers around borderless inputs — search box, command palette, timezone search). `:focus-visible`, not `:focus`, so a mouse click never leaves a ring behind. Don't re-suppress it per component.

### Accessibility

Kept as a standing floor, not a one-off pass: a `.skip-link` jumps past the ten sidebar buttons to `<main id="app-content">`; the sidebar is a labelled `<aside>`; sortable table headers are focusable and sort on Enter/Space with `aria-sort` on the `<th>` itself; the toast host is a permanently-mounted `role="status" aria-live="polite"` region (a live region that appears *with* its text is not reliably announced — which is why the individual toasts carry no role of their own). Icon-only buttons carry `aria-label`. The same rules apply to the exported interactive report.
...
`saveTextExport` / `saveBinaryExport` (App.tsx) hide the desktop/web split: Electron gets a native Save As via `window.desktopExport`, the web build downloads a blob. PDF is real `printToPDF` in an offscreen window on desktop, and an offscreen-iframe print dialog on web. All free text going into HTML/Word reports goes through `escapeHtml()` — **including values written into `data-*` attributes**, which the interactive report leans on.

Five formats: Excel, CSV, PDF, Word, and a self-contained **interactive `.html`** (charts drawn, table searchable/filterable/sortable, count strip re-totalling what shows — no network, no libraries). Every report carries charts, built by `lib/report.ts` off `lib/reportchart.ts` since Recharts needs a live React tree a file on disk doesn't have. **Two renderings, deliberately:** real SVG for the PDF and the interactive HTML (browser engines draw them), and coloured table cells (`htmlBarRows`) for Word — Word's HTML importer drops inline SVG *silently*, so a "chart" there would just be missing. Don't unify them. `xlsx` is imported on demand inside the Excel exporter, not at module scope. It installs from a `cdn.sheetjs.com` tarball URL, not the npm registry (abandoned at 0.18.5 with open advisories) — never revert it to a semver range; see [KNOWN_ISSUES.md](KNOWN_ISSUES.md) non-issues.
