...
**Import direction is one-way: `App.jsx` → `lib/*`.** `Charts.jsx`, `lib/format.ts` and `lib/trade.ts` must never import `App.jsx` — that circular import drags the whole app into the lazy chart chunk and defeats the code split. Enforced by a `no-restricted-imports` rule in `eslint.config.js`, which applies to `lib/**/*.ts` the same as `.js`/`.jsx`.

New pure logic goes in `lib/trade.ts` with a test, not in App.jsx. Anything touching `window`, `document` or `storage` stays in App.jsx.

### Type safety

Incremental, not a rewrite (batch #4 of [UPGRADES.md](UPGRADES.md)): `src/lib/*.ts` (`trade.ts`, `auth.ts`, `format.ts`) are real TypeScript, `strict: true`, checked by `tsc --noEmit` (chained into `npm run lint`, and its own `npm run typecheck`). `App.jsx` and `Charts.jsx` stay plain `.jsx` for now — `tsconfig.json` has `allowJs: true, checkJs: false`, so they're included in the program (imports from them resolve, and importing the typed `lib/*` gives IDE intellisense inside them) without being type-checked. No import-path changes were needed anywhere: this codebase already imports extension-less (`from "./lib/trade"`), which resolves identically to a `.ts` or `.js` file.

Boundary functions that exist specifically to coerce "whatever's on disk" — `mergeSettings`, `mergePreferences`, `normalizeAccounts`, `normalizeUsers`, `rowsToTrades`, `computeTrade`'s input — take `unknown`/loose input on purpose and return one of the domain types (`Trade`, `ComputedTrade`, `Settings`, `Preferences`, `Account`, `AuthUser`, …) defined at the top of `trade.ts` / `auth.ts`. That is intentional, not a gap: the whole point of those functions is "accept anything, guarantee a shape coming out." Don't tighten their parameter types to the domain types themselves — that would just move the `as`-casting into every call site instead of the one function whose job is to absorb it.

`tsconfig.json` excludes `electron/` — `main.cjs`/`preload.cjs` are a separate CommonJS/Node program (electron-builder's target), not part of the Vite/React app this migration covers.

**Not yet done, deliberately deferred:** `App.jsx` (~5.2k lines) and `Charts.jsx` staying JS. A future session can either JSDoc-type them in place (`checkJs: true` per-file via `// @ts-check`) or migrate `App.jsx` → `.tsx` outright — see UPGRADES.md batch #4 for the two options and their tradeoffs. Whichever is chosen, `src/vite-env.d.ts` already declares the ambient `__APP_VERSION__` global either file type needs.
